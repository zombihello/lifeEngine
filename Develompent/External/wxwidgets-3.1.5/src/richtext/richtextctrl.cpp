...
bool wxRichTextCtrl::MoveRight(int noPositions, int flags)
{
    ...
    if (flags & wxRICHTEXT_SHIFT_DOWN)
    {
        if (m_selection.GetContainer() && m_selection.GetContainer()->IsKindOf(wxCLASSINFO(wxRichTextTable)))
        {
    ...
    if (beyondBottom || beyondTop)
    {
            ...
            )
        {
            ...
            {
                if ((flags & wxRICHTEXT_SHIFT_DOWN) &&
                    GetFocusObject()->IsKindOf(wxCLASSINFO(wxRichTextCell)) &&
                    actualContainer->IsKindOf(wxCLASSINFO(wxRichTextCell)) &&
                ...
                if (actualContainer->IsKindOf(wxCLASSINFO(wxRichTextCell)))
                {
                    if (beyondBottom)
                        newPos = 0;
                    else
