...
bool CConsoleSystem::Exec( const std::wstring& InCommand )
{
	...
	// Find by name variable, if we found change him value
	{
		...
		if ( var )
		{
			...
			else
			{
				...
				LE_LOG( LT_Log, LC_General, TEXT( "%s: %s" ), name.c_str(), var->GetHelpText().c_str() );
				LE_LOG( LT_Log, LC_General, TEXT( "Value: %s" ), stringVar.c_str() );
				LE_LOG( LT_Log, LC_General, TEXT( "Default value: %s" ), var->GetValueDefault().c_str() );
			}
	...
	LE_LOG( LT_Error, LC_General, TEXT( "Not correct command: \"%s\"" ), InCommand.c_str() );
	return false;
}
...
void CConsoleSystem::CmdHelp( const std::vector<std::wstring>& InArguments )
{
	...
	LE_LOG( LT_Log, LC_General, TEXT( "" ) );
	LE_LOG( LT_Log, LC_General, TEXT( "** Console variables **" ) );
	{
		std::vector<CConVar*>&		vars = GetConVars();
		for ( uint32 index = 0, count = vars.size(); index < count; ++index )
		{
			CConVar*	var = vars[index];
			LE_LOG( LT_Log, LC_General, TEXT( "%s : %s. Default value: %s" ), var->GetName().c_str(), var->GetHelpText().c_str(), var->GetValueDefault().c_str() );
		}
	}

	// Print all console commands
	LE_LOG( LT_Log, LC_General, TEXT( "" ) );
	LE_LOG( LT_Log, LC_General, TEXT( "** Console commands **" ) );
	{
		std::vector<CConCmd*>&		cmds = GetConCmds();
		for ( uint32 index = 0, count = cmds.size(); index < count; ++index )
		{
			CConCmd*	cmd = cmds[index];
			LE_LOG( LT_Log, LC_General, TEXT( "%s : %s" ), cmd->GetName().c_str(), cmd->GetHelpText().c_str() );
		}
	}
}