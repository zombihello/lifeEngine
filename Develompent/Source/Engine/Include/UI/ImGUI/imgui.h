...
namespace ImGui
{
    ...
    IMGUI_API bool          IsWindowHovered(ImGuiHoveredFlags flags=0); // is current window hovered (and typically: not blocked by a popup/modal)? see flags for options. NB: If you are trying to Assert whether your mouse should be dispatched to imgui or to your app, you should use the 'io.WantCaptureMouse' boolean for that! Please read the FAQ!
...
// Note: if you are trying to Assert whether your mouse should be dispatched to Dear ImGui or to your app, you should use 'io.WantCaptureMouse' instead! Please read the FAQ!
...
struct ImVector
{
    ...
    inline ImVector<T>& operator=(const ImVector<T>& src)   { clear(); resize(src.Size); Memory::Memcpy(Data, src.Data, (size_t)Size * sizeof(T)); return *this; }
    ...
    inline void         resize(int new_size, const T& v)    { if (new_size > Capacity) reserve(_grow_capacity(new_size)); if (new_size > Size) for (int n = Size; n < new_size; n++) Memory::Memcpy(&Data[n], &v, sizeof(v)); Size = new_size; }
    inline void         shrink(int new_size)                { IM_ASSERT(new_size <= Size); Size = new_size; } // Resize a vector to a smaller size, guaranteed not to cause a reallocation
    inline void         reserve(int new_capacity)           { if (new_capacity <= Capacity) return; T* new_data = (T*)IM_ALLOC((size_t)new_capacity * sizeof(T)); if (Data) { Memory::Memcpy(new_data, Data, (size_t)Size * sizeof(T)); IM_FREE(Data); } Data = new_data; Capacity = new_capacity; }

    // NB: It is illegal to call push_back/push_front/insert with a reference pointing inside the ImVector data itself! e.g. v.push_back(v[10]) is forbidden.
    inline void         push_back(const T& v)               { if (Size == Capacity) reserve(_grow_capacity(Size + 1)); Memory::Memcpy(&Data[Size], &v, sizeof(v)); Size++; }
    inline void         pop_back()                          { IM_ASSERT(Size > 0); Size--; }
    inline void         push_front(const T& v)              { if (Size == 0) push_back(v); else insert(Data, v); }
    inline T*           erase(const T* it)                  { IM_ASSERT(it >= Data && it < Data + Size); const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + 1, ((size_t)Size - (size_t)off - 1) * sizeof(T)); Size--; return Data + off; }
    inline T*           erase(const T* it, const T* it_last){ IM_ASSERT(it >= Data && it < Data + Size && it_last > it && it_last <= Data + Size); const ptrdiff_t count = it_last - it; const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + count, ((size_t)Size - (size_t)off - count) * sizeof(T)); Size -= (int)count; return Data + off; }
    inline T*           erase_unsorted(const T* it)         { IM_ASSERT(it >= Data && it < Data + Size);  const ptrdiff_t off = it - Data; if (it < Data + Size - 1) Memory::Memcpy(Data + off, Data + Size - 1, sizeof(T)); Size--; return Data + off; }
    inline T*           insert(const T* it, const T& v)     { IM_ASSERT(it >= Data && it <= Data + Size); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(_grow_capacity(Size + 1)); if (off < (int)Size) memmove(Data + off + 1, Data + off, ((size_t)Size - (size_t)off) * sizeof(T)); Memory::Memcpy(&Data[off], &v, sizeof(v)); Size++; return Data + off; }
...
struct ImGuiWindowClass
{
    ...
    ImGuiWindowClass() { Memory::Memset(this, 0, sizeof(*this)); DockingAllowUnclassed = true; }
};
...
struct ImGuiPayload
{
    ...
    ImGuiPayload()  { Clear(); }
    void Clear()    { SourceId = SourceParentId = 0; Data = NULL; DataSize = 0; Memory::Memset(DataType, 0, sizeof(DataType)); DataFrameCount = -1; Preview = Delivery = false; }
...
struct ImGuiTableColumnSortSpecs
{
    ...
    ImGuiTableColumnSortSpecs() { Memory::Memset(this, 0, sizeof(*this)); }
};
...
struct ImGuiTableSortSpecs
{
    ...
    ImGuiTableSortSpecs()       { Memory::Memset(this, 0, sizeof(*this)); }
};
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    inline ImGuiListClipper(int items_count, float items_height = -1.0f) { Memory::Memset(this, 0, sizeof(*this)); ItemsCount = -1; Begin(items_count, items_height); } // [removed in 1.79]
#endif
};
...
struct ImDrawCmd
{
    ...
    ImDrawCmd() { Memory::Memset(this, 0, sizeof(*this)); } // Also ensure our padding fields are zeroed
...
struct ImDrawListSplitter
{
    ...
    inline ImDrawListSplitter()  { Memory::Memset(this, 0, sizeof(*this)); }
...
struct ImDrawList
{
    ...
    ImDrawList(const ImDrawListSharedData* shared_data) { Memory::Memset(this, 0, sizeof(*this)); _Data = shared_data; }
    ...
    inline    void  PathLineToMergeDuplicate(const ImVec2& pos)                 { if (_Path.Size == 0 || Memory::Memcmp(&_Path.Data[_Path.Size - 1], &pos, 8) != 0) _Path.push_back(pos); }
    ...
    // Advanced
    IMGUI_API void  AddCallback(ImDrawCallback callback, void* callback_data);  // Your rendering function must Assert for 'UserCallback' in ImDrawCmd and call the function instead of rendering triangles.
...
struct ImDrawData
{
    ...
    // Functions
    ImDrawData()    { Clear(); }
    void Clear()    { Memory::Memset(this, 0, sizeof(*this)); }     // The ImDrawList are owned by ImGuiContext!
...
struct ImFontGlyphRangesBuilder
{
    ...
    inline void     Clear()                 { int size_in_bytes = (IM_UNICODE_CODEPOINT_MAX + 1) / 8; UsedChars.resize(size_in_bytes / (int)sizeof(ImU32)); Memory::Memset(UsedChars.Data, 0, (size_t)size_in_bytes); }
...
struct ImFontAtlas
{
    ...
    bool                        IsBuilt() const             { return Fonts.Size > 0 && TexReady; } // Bit ambiguous: used to detect when user didn't built texture but effectively we should Assert TexID != 0 except that would be backend dependent...
...
struct ImGuiViewport
{
    ...
    ViewportRHIRef_t     ViewportRHI;            // Pointer to viewport RHI for renderer (e.g. swap chain, framebuffers etc.). generally set by your Renderer_CreateWindow function.
    ...
    ImGuiViewport()     { Memory::Memset(this, 0, sizeof(*this)); }
    ~ImGuiViewport()    { IM_ASSERT( PlatformUserData == NULL ); }
...
// - You can Assert its simple source code to understand what it does.
...
struct ImGuiPlatformIO
{
    ...
    ImGuiPlatformIO()               { Memory::Memset(this, 0, sizeof(*this)); }     // Zero clear
};
...
namespace ImGui
{
    ...
    // For shared code, you can version Assert at compile-time with `#if IMGUI_VERSION_NUM >= 17704`.
