...
// BG yehor.pohuliaka Begin - Implement ImGUI in lifeEngine
void appImGUIProcessEvent( struct SWindowEvent& InWindowEvent )
{
	...
	{
		if ( InWindowEvent.events.mouseButton.code == BC_MouseLeft )	g_MousePressed[ 0 ] = true;
		if ( InWindowEvent.events.mouseButton.code == BC_MouseRight )	g_MousePressed[ 1 ] = true;
		if ( InWindowEvent.events.mouseButton.code == BC_MouseMiddle )	g_MousePressed[ 2 ] = true;
		break;
	}
	...
	case SWindowEvent::T_KeyReleased:
	{
		...
		imguiIO.KeySuper		= InWindowEvent.events.key.isSuper;
		break;
	}
	}
}
// BG yehor.pohuliaka End

bool ImGui_ImplSDL2_Init( SDL_Window* window )			// BG yehor.pohuliaka - Removed operator 'static'
{
	g_Window = window;
...
{
	check( GWindow );
	return ImGui_ImplSDL2_Init( static_cast< FWindowsWindow* >( GWindow )->GetSDLWindow() );
}
