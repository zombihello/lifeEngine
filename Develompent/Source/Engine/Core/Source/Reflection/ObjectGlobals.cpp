...
static bool						s_bIsSavingPackage = false;
...
CObjectPackage* CreatePackage( CObject* InOuter, const tchar* InPackageName )
{
	...
	if ( !resultPackage )
	{
		return new( InOuter, InPackageName ) CObjectPackage;
	}
	else
	{
...
{
	ObjectSerializeContext&		objectSerializeContext = CLinkerLoad::GetObjectSerializeContext();
...
void EndLoad()
{
	CLinkerLoad::GetObjectSerializeContext().DecrementBeginLoadCount();
}
...
bool SavePackage( CObjectPackage* InOuter, CObject* InBase, ObjectFlags_t InTopLevelFlags, const tchar* InFilename )
{
...

	{
		...
		TGuardValue<bool>	isSavingFlag( s_bIsSavingPackage, true );
		...
		InOuter->SetGuid( linker.GetSummary().guid );
...
{
	// TODO yehor.pohuliaka: Need implement
	AssertNoEntry();
	return nullptr;
}