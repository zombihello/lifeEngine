...
==================
*/
std::string CString::Format( const achar* InFormat, ... )
{
	va_list			arguments;
	va_start( arguments, InFormat );

	std::string	formatedString = Format( InFormat, arguments );
...
==================
*/
std::string CString::Format( const achar* InFormat, va_list InArguments )
{
	int32           bufferSize = 1024;
	...
	while ( result == -1 )
	{
		...
		// Get formated string with args
		result = Sys_GetVarArgsAnsi( buffer, bufferSize, bufferSize - 1, InFormat, InArguments );
		if ( result >= bufferSize )
		{
			result = -1;
		}
